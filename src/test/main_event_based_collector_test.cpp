...
bool SingleEntryZmqDoocs::GetExpectedSizesAndCreateBuffers()
{
    ...
    case DATA_INT:
        m_expectedRead1 = static_cast<size_t>(m_nCount) * sizeof(int);
        break;

    case DATA_FLOAT:
        m_expectedRead1 = static_cast<size_t>(m_nCount) * sizeof(float);
        break;
    ...
    case DATA_SPECTRUM:
        ...
        m_expectedRead1 = sizeof(struct SPECTRUM) + STRING_LENGTH ;
        m_expectedRead2 = static_cast<size_t>(m_nKnownCount) * sizeof(float);  // in order to keep socket clean
        break;
    ...
    if(m_expectedRead1){
        m_pBuffer1 = static_cast<char*>(malloc(m_expectedRead1));
    }
