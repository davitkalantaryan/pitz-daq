//
// file eq_pitzrf1fpga.cc
//
// pitzrf1fpga Eq function class
...
const char*	object_name = "pitzrf1fpga";

EqFct*		pitzrf1fpga_fct;

int	EqFctpitzrf1fpga::conf_done = 0; 
...
EqFctpitzrf1fpga::EqFctpitzrf1fpga() : 
		...
		,serverstatus_("SERVER_STATUS ok, when =11",this)
{
   ...
   D_fct* t;
	...
	t = &alias_ ; 		        fct_list->push_back ( t );
	t = &Command_ ; 		     fct_list->push_back ( t );
	t = &ThreadStatus_; 		  fct_list->push_back ( t );
	t = &Conds_; 		  		  fct_list->push_back ( t );	
	t = &root_length;         fct_list->push_back ( t );
	t = &gen_event_;   		  fct_list->push_back ( t );
	
	t = &Alarm_;   	   	  fct_list->push_back ( t );	
	t = &WriteFile_;   	     fct_list->push_back ( t );	
	
	t = &datastream_;   		  fct_list->push_back ( t );
	t = &serverstatus_;   	  fct_list->push_back ( t );	
#endif
...
void	EqFctpitzrf1fpga::init()
{
	...
	parse_condition();	
        ...
        if( m_threadStatus.value() == 0 ) { exit(1); }
...
EqFct* eq_create(int eq_code, void*)
{
   ...
   switch (eq_code) 
	{
   	case Codepitzrf1fpga:
      	eqn = new EqFctpitzrf1fpga();
      break;
      default:
         eqn = (EqFct *) 0;
      break;
...
void EqFctpitzrf1fpga::parse_config()
{
        bool bDelete;
        int nRecipts(0);
	...
	else
	{
        ...
        if ( StatusConfig == 0 ) m_threadStatus.set_value(0);
...
void EqFctpitzrf1fpga::parse_condition()
{
	StatusCondition = 1;
	...
	else
	{
        ...
        if ( StatusCondition == 0 ) m_threadStatus.set_value(0);
	return ;
}
...
{
	EqFctpitzrf1fpga* eq_fct_ = (EqFctpitzrf1fpga*) arg;
	...
	pthread_sigmask( SIG_BLOCK, &sigset, NULL );	
        ...
        while( eq_fct_->m_threadStatus.value() == 1 )
	{
...
void *fill_struct(void *arg)
{
	...
	pthread_sigmask( SIG_BLOCK, &sigset, NULL );			
        ...
        while( message->eq_fct_->m_threadStatus.value() == 1 )
	{
			...
			if( StatusCondition == 1 )
			{
				...
				if(dst->error() == 0) 	
				{
					switch ( message->type ) 
					{
						...
						break;										
						case 19 :
							f = dst->get_float_array();
							for(j=0; j < samples; j++)						
							{ 
								 transit[message->id][message->carS]->array_value[j] = f[j];
							}
							transit[message->id][message->carS]->buffer = GENSTATIC;
							transit[message->id][message->carS]->time   = SECSTATIC;
					...
					{
                                                if( (message->eq_fct_->m_alarm.value()) == 1 ) { message->eq_fct_->m_alarm.set_value(2); }
					}					
					
				}
				else
				{
                                        if( (message->eq_fct_->m_alarm.clean == 0) && (message->eq_fct_->m_nWriteFile == 0) )
					{
						NOALARM_COUNTER = 0;
						...
						for(int i=0; i < 6; i++)
						{
							...
							if( strcmp(message->statistic_time,"") != 0) { break; }	
                                                ...
                                                message->eq_fct_->m_alarm.set_value(1);
					}
					nanosleep(&ts, NULL);
				}					
			}
...
void* root_thread(void *arg)
{
	...
	{
                if( eq_fct_->m_threadStatus.value() == 0 ) { exit(0); }
		nanosleep(&ts, NULL); 
	}		
...
//		sprintf(write_dir,"/export/doocs/daq/daqL");	
		...
		for(i=0; i < CONFIG_COUNTER; i++)
		{
			switch ( DaqConf[i]->type ) 
			{
				...
				case 19 :
					file->cd(); gFile = file;				
					tree[i] = new TTree(DaqConf[i]->daq_name, "DATA");
					tree[i]->Branch(DaqConf[i]->daq_name, &(D_C19[i]->time),"time/I:buffer/I:array_value[2048]/F");					
				break;
				default :
				break;								
			}				
		...
		Close_File_R = 0;
                ...
                while( (Close_File_R == 0) && (eq_fct_->m_threadStatus.value() == 1) )
		{
			for(i=0; i < CONFIG_COUNTER; i++)
			{			
				if( DaqConf[i]->carR != DaqConf[i]->carS )
				{
					...
					switch ( DaqConf[i]->type ) 
					{
						...
						break;																		
						case 19 :
							D_C19[i]->time        = (Int_t)(transit[i][(DaqConf[i]->carR)]->time);
							D_C19[i]->buffer      = (Int_t)(transit[i][(DaqConf[i]->carR)]->buffer);
							for(j=0; j < 2048; j++) 
							{ 
								 D_C19[i]->array_value[j] = (Float_t)(transit[i][(DaqConf[i]->carR)]->array_value[j]); 
							}
		...
		if( GE != GENSTATIC )
		{
			...
			{ 
				board_channel[i] = 1;
                        ...
                        if( (DaqConf[0]->eq_fct_->m_threadStatus.value()) == 1 )
			{
...
void* stat_thread(void * arg)
{
	...
	{		
		...
		{
                        eq_fct_->m_alarm.set_value(2);
		}
	}

        while( eq_fct_->m_threadStatus.value() == 1 )
	{
                if(eq_fct_->m_alarm.clean == 1)
		{
			nanosleep(&ts, NULL);
			...
			fclose(statistic_ptr);
                        ...
                        eq_fct_->m_alarm.clean = 0;
		}
		
                if( (eq_fct_->m_nWriteFile == 1) && (eq_fct_->m_alarm.value() > 0) )
		{
			nanosleep(&ts, NULL);		
